...
Gating inside `init()` itself, rather than at whatever calls it, is what lets this join [`application-init`](../application-init)'s auto-discovered `*.service.ts` set: `initServices` calls every matched service's `init()` unconditionally, with no per-service logic of its own deciding which ones actually apply this session — each service has to know that about itself, the same way [`device-type-detection`](../device-type-detection) already does.
...
`touchstart` records `startY` only when the page hasn't been scrolled down (`window.pageYOffset > 0` skips it — loose enough to also let through the negative values iOS produces during elastic overscroll past the top, not just an exact `0`) — starting a drag anywhere else isn't a pull-to-refresh gesture, it's just scrolling. `touchend` computes how far the finger moved (`deltaY`) and reloads only if it crossed `REFRESH_THRESHOLD` (200px) while the page is *still* not scrolled down when the touch ends. There's no visual feedback during the drag (no pull-down spinner or progress indicator tracking finger position) — this fires the reload or does nothing, all-or-nothing at release, rather than a continuously-tracked gesture. A production pull-to-refresh with a visual indicator would need to track `touchmove` too, not just start/end. `enable()` is guarded by a module-level `isEnabled` flag, so nothing stops `init()` itself from running more than once — a stray manual call, an HMR reload picking the module back up — without that guard attaching a second, independent set of listeners that would each reload the page off the same gesture.
...
- [`progressive-web-app`](../progressive-web-app) — `display: 'standalone'` in that entry's manifest is what actually puts an iOS home-screen install into the standalone mode this gesture is gated on; without that setting there'd be no standalone launch to lose the native gesture in the first place.
- [`application-init`](../application-init) — its services step auto-discovers this the same way it does [`device-type-detection`](../device-type-detection): by file name and an exported `init()`, nothing else wiring it in by hand.
